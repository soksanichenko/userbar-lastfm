...
def gradient(
    inner_color: RGBColor,
    outer_color: RGBColor,
...
def paste_text(
    ...
    if truncate_text:
        text = f"{text[:trunc_len]}..." if len(text) > trunc_len else text
...
def make_userbar_text(
    text_format: str = "{artist_name} - {track_name}", **kwargs: object
) -> str:
...
def create_userbar(
    ...
    img = paste_text(
        text, text_color, inner_color, outer_color, border_color, truncate_text
    )
    img.seek(0)
