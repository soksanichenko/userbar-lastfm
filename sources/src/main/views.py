...
from src.utils.api import LastFmException, User
from src.utils.main import create_userbar
from src.utils.nocache import nocache
from src.utils.utils import hex_to_rgb_tuple, parse_color_string

logger = logging.getLogger(__name__)

_COLOR_KEYS = frozenset(
    {"inner_color", "outer_color", "text_color", "border_color", "logo_color"}
)
...
def generator() -> str:
    ...
    return render_template(
        "format1.html", title="Last.Fm UserBar Generator", root_url=request.url_root
    )
...
def main() -> str:
    ...
    return render_template(
        "main.html", title="Last.Fm UserBar", root_url=request.url_root
    )
...
@nocache
def user_bar(
    username: str, inner_color: str, outer_color: str, text_color: str
) -> Response:
...
def new_user_bar(username: str, params: str | None = None) -> Response:
    """Render a userbar PNG; colours passed as underscore-RGB query params."""
