...
class User(LastFmApi):
    def __init__(self, api_key: str, url: str = API_URL, raw: bool = False,
                 username: str | None = None) -> None:
        super().__init__(api_key, url, raw)
        assert username is not None, 'User is not defined'
        self.username = username

    def user_get_recent_tracks(self, limit: int = 1, page: int = 1, extended: int = 0,
                               current: bool = False, from_time: int | None = None,
                               to_time: int | None = None, raw: bool | None = None) -> dict:
    ...
    def user_get_info(self, raw: bool = False) -> dict:
        ...
        if 'user' in result:
            ...
            user_info['user_info']['registered'] = (  # type: ignore[index]
                datetime.fromtimestamp(int(u['registered']['unixtime']))
                .strftime('%d-%m-%Y %H:%M:%S')
            )
        return user_info

    def user_get_artist_tracks(self, raw: bool = False, artist: str | None = None,
                               from_time: int | None = None, to_time: int | None = None,
                               page: int = 1) -> dict:
